(async () => {
    ...
    const versions = ["maimai", "maimai PLUS"].concat(
        (await fetch(`${siteOrigin}/mai-tools/json/version.json`)
            .then(res => res.json()))
            .map(function (e) {
                if (!e.versionName.includes("~")) { return e.versionName }
            }))
    ...
    for (let i = 0; i < versions.length; i++) {
        const res = await fetch(`https://maimaidx-eng.com/maimai-mobile/record/musicVersion/search/?version=${i}&diff=2`, {
            credentials: 'include'
        });
        const text = await res.text();
        const doc = new DOMParser().parseFromString(text, 'text/html');

        const blocks = doc.querySelectorAll('.music_expert_score_back.pointer.w_450.m_15.p_3.f_0');
