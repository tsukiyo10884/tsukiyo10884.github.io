...
const createSpecialButtons = () => {
    ...
    const $button1 = $('<button>')
        .html('<img src="./img/special/axuan_icon.webp" alt="阿瑄專屬" style="height: 50px;">')
        .css({
            'padding': '0px'
        })
    ...
    const $button2 = $('<button>')
        .html('<img src="./img/special/ayo_icon.webp" alt="阿幽專屬" style="height: 50px;">')
        .css({
            'padding': '0px'
        })
        .attr('data-bs-toggle', 'tooltip')
...
const initForAyoList = () => {
    ...
    const $profile = $('<img id="ayo_profile">')
        .attr('src', './img/special/ayo_profile.webp')
...
const flys = () => {
    for (let i = 0; i < 3; i++) {
        let $img = $('<img class="fly">').attr('src', `./img/special/ayo_mini.webp`);
...
const initForAXuanList = () => {
    ...
    const $profile = $('<img id="axuan_profile">')
        .attr('src', './img/special/axuan_profile.webp')
        .attr('style', 'position: relative;z-index:99;width: 422px;')
        .on('click', function () {
            $('.basic_block').show();
            $profile.remove();
