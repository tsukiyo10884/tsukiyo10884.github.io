import * as Gate from './js/fetch_data/fetch_data_gate.js';
...
(async () => {
    ...
    if (type == 'main' || type == 'friend') {
        ...
        switch (no) {
            ...
            case 4:
            case 6:
                gate = await Gate.getGateUnlockProgress(domain, no);
                break;
            ...
            case 3:
            case 5:
                gate = await Gate.getGateRequirements(domain, no);
                break;

            // 塔
            case 7: {
                const gateData = await Gate.fetchGateStatus(domain, 7);
